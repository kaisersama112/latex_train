...
if __name__ == '__main__':
    ...
    dataset = load_dataset('./dataset/loader.py', trust_remote_code=True)['train']

    dataset = dataset.filter(lambda x: x['image'].height > MIN_HEIGHT and x['image'].width > MIN_WIDTH)
    dataset = dataset.shuffle(seed=42)
    dataset = dataset.flatten_indices()
    # 默认tokenizer
    tokenizer = TexTeller.get_tokenizer(r'D:\model_train_list_project\latex_train\src\models\ocr_model\TexTeller')
    # 重新构建的tokenizer 但是词汇不够
    # tokenizer = TexTeller.get_tokenizer(r'D:\model_train_list_project\latex_train\src\models\self_tokenizer_dir')
    ...
    # +)
    # 基于预训练模型进行训练
    model = TexTeller.from_pretrained(r'D:\model_train_list_project\latex_train\src\models\ocr_model\TexTeller')
    enable_train = True
